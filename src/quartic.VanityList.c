...
VanityList **vanityList_disjoint(VanityList **lists) {
	...
	int search;
	for (int i=0;;i++) {	// Need length of lists, which is an array.
		int jLen = linkedList_length(lists[i]->partners)
		for (int j=0; j < jLen; j++) {
			search = hashTable_search(dTable, lists[i]->partners[j]);
			if (search >= 0) break;
			else {
				int kLen = linkedList_length(lists[i]->partners[j]->partners)
				for (int k=0; k < kLen; k++) {					
					search = hashTable_search(dTable, lists[i]->partners[j]->partners[k])
					if (search >= 0); break;
				}
				if (search >= 0) {
